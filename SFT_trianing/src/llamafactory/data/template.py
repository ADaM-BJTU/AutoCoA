...
@dataclass
class PartialPreferenceTemplate(Template):
    @override
    def _encode(
        self,
        tokenizer: "PreTrainedTokenizer",
        ...
        for i, message in enumerate(messages):
            ...
            _format_messages.append(elements)
            # encoded_messages.append(self._convert_elements_to_ids(tokenizer, elements))

        # we only process the last turn because the previous turns are must complete
        for elements in _format_messages[:-2]:
            encoded_messages.append(self._convert_elements_to_ids(tokenizer, elements))
        
        last_turn_elements = _format_messages[-2:]
        assert isinstance(last_turn_elements[0], list) and isinstance(last_turn_elements[1], list)
        last_turn_input, last_turn_output = last_turn_elements[0][1], last_turn_elements[1][0]
        # <｜User｜>{{content}}<｜Assistant｜><think>\n
        # last_turn_real_output = last_turn_output.split('<think>\n')[-1].removesuffix(tokenizer.eos_token)
        last_turn_real_output = last_turn_output

        remove_generation_prompt_last_turn_input = last_turn_input.split('<｜Assistant｜>')[0]
        last_turn_input_user_part = remove_generation_prompt_last_turn_input.rsplit('<think>', 1)[0]
        last_turn_input_common_prefix = remove_generation_prompt_last_turn_input.rsplit('<think>', 1)[-1]
        last_turn_real_input = f'{last_turn_input_user_part}<｜Assistant｜><think>\n{last_turn_input_common_prefix}'

        last_turn_elements[0][1], last_turn_elements[1][0] = last_turn_real_input, last_turn_real_output
        _ = last_turn_elements[1].pop(-1)
        for elements in last_turn_elements:
            encoded_messages.append(self._convert_elements_to_ids(tokenizer, elements))
...
register_template(
    name="deepseek3_add-think",
    format_user=StringFormatter(slots=["<｜User｜>{{content}}<｜Assistant｜><think>\n"]),
    format_prefix=EmptyFormatter(slots=[{"bos_token"}]),
)

register_template(
    name="deepseek3_add-think_ppf",
    format_user=StringFormatter(slots=["<｜User｜>{{content}}<｜Assistant｜><think>\n"]),
    format_prefix=EmptyFormatter(slots=[{"bos_token"}]),
    template_class=PartialPreferenceTemplate,
)
