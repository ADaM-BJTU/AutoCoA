...
from flashrag.utils import get_reranker, get_logger
...
def timer(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        logger.info(f"{func.__name__} cost time: {end - start:.2f}s")
        return result
    return wrapper
...
class DenseRetriever(BaseTextRetriever):
    ...
    def load_index(self):
        ...
        vectors_fp16 = self._reconstruct_as_fp16(cpu_index)
        dim = vectors_fp16.shape[-1]
        gpu_index = faiss.index_factory(dim, 'Flat', faiss.METRIC_INNER_PRODUCT)
        co = faiss.GpuMultipleClonerOptions()
        co.useFloat16 = True
        co.shard = True
        gpu_index = faiss.index_cpu_to_all_gpus(gpu_index, co)
        if not gpu_index.is_trained:
            gpu_index.train(vectors_fp16)
        gpu_index.add(vectors_fp16)
    ...
    @timer
    def _reconstruct_as_fp16(self, index):
        n = index.ntotal
        d = index.d
        ...
        logger.info(f"Converted vectors to float16, shape: {vectors_fp16.shape}, dtype: {vectors_fp16.dtype}")
    ...
    def load_model(self):
        ...
        print("Model loaded successfully in deivce: {}".format(self.encoder.model.device))
    ...
    def _batch_search(self, query: List[str], num: int = None, return_score=False):
        ...
        # index_search_end = time.time()
        # print("End faiss searching. Search time: {:.2f}s".format(index_search_end - index_search_start))
