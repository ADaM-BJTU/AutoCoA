...
def load_corpus(corpus_path: str):
    ...
    if 'contents' not in corpus.features:
        try:
            ...
            corpus = corpus.map(lambda x: {"contents": f'{fix_title_quotes(x["title"])}\n{x["text"].strip()}'})
        except:
...
def load_docs(corpus, doc_idxs: List[int]):
    ...
    results = []
    for idx in doc_idxs:
        _entry = corpus[int(idx)]
        # title, text = _entry["title"], _entry["text"].strip()
        contents = _entry["contents"]
        # _result = dict(id=str(_entry["id"]), contents=f'{title}\n{text}')
        _result = dict(id=str(_entry["id"]), contents=contents)
