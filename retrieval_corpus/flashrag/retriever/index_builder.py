...
class Index_Builder:
    ...
    def __init__(
        ...
        logger.info(f'Loaded {len(self.corpus)} documents from {self.corpus_path}')
    ...
    def encode_all(self):
        ...
        # self.corpus = self.corpus.select(range(100000))
        encode_data = []
        for item in tqdm(self.corpus, desc=f'Loading documents from corpus to encode ...'):
            encode_data.append(item["contents"])            
        if self.gpu_num > 1:
            logger.info("Use multi gpu!")
    ...
    def build_dense_index(self):
        ...
        if self.embedding_path is not None:
            ...
            logger.debug(f'embedding shape: {all_embeddings.shape}, dtype: {all_embeddings.dtype}')
